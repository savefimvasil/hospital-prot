<template>
  <v-form v-model="valid">
    ...
    <v-text-field
      ...
      required
    />
    <v-btn :disabled="!valid" color="info" :justify-end="true" @click="registerUser">Зареєструвати</v-btn>
  </v-form>
</template>
    ...
    methods: {
      ...
      registerUser() {
        let userData = {
          ...
          store: this.$store.state.user.user
        }
        this.$store.dispatch('user/registerDoctor', userData)
          .then(() => {
            this.$router.push('/')
          })
          .catch(error => {
